...
template<typename T>
__global__ void BlurSharedKernel(T * img_in, T * img_out, int h_i, int w_i, T * filt_in, int w_f) {
    __shared__ float shared_filter[CONSTMEMSIZE];

    if (threadIdx.x == 0 && threadIdx.y == 0 && threadIdx.z == 0) {
        for(int ii = 0 ; ii < w_f * w_f ; ii++) {
            shared_filter[ii] = filt_in[ii];
            // printf("%d -> %f\n", ii, shared_filter[ii]);
        }
    }
    __syncthreads();
            ...
            for (int cc = 0; cc < w_f; cc++) {
                ...
                for (int rr = 0; rr < w_f; rr++) {
                    ...
                    if (in_row >= 0 && in_row < h_i && in_col >= 0 && in_col < w_i) {
                        p_value += shared_filter[rr * w_f + cc] * img_in[in_row * w_i + in_col];
                    }
                }
            }
